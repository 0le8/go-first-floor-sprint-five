...
func (r Running) Calories() float64 {
	...
	return (float64(CaloriesMeanSpeedMultiplier) * r.meanSpeed() + CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours() * MinInHours
}
...
func (w Walking) Calories() float64 {
	...
	return (CaloriesWeightMultiplier * w.Weight + (math.Pow(meanSpeedInMSec, 2) / heightInM) * CaloriesSpeedHeightMultiplier *  float64(w.Weight) ) * w.Duration.Hours() * MinInHours
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	ret := s.Training.TrainingInfo()
	ret.Speed = s.meanSpeed()
